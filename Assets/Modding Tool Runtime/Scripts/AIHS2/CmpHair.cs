        ...
        public override void SetReferenceObject()
        {
            ...
            for (var i = 0; i < boneInfo.Length; i++)
            {
                ...
                if (!keyValuePair2.Equals(default(KeyValuePair<string, GameObject>)))
                {
                    ...
                    boneInfoInstance.moveRate.x = Mathf.InverseLerp(boneInfoInstance.posMin.x,
                        boneInfoInstance.posMax.x, boneInfoInstance.basePos.x);
                    boneInfoInstance.moveRate.y = Mathf.InverseLerp(boneInfoInstance.posMin.y,
                        boneInfoInstance.posMax.y, boneInfoInstance.basePos.y);
                    boneInfoInstance.moveRate.z = Mathf.InverseLerp(boneInfoInstance.posMin.z,
                        boneInfoInstance.posMax.z, boneInfoInstance.basePos.z);
                    boneInfoInstance.rotRate.x = Mathf.InverseLerp(boneInfoInstance.rotMin.x, boneInfoInstance.rotMax.x,
                        boneInfoInstance.baseRot.x);
                    boneInfoInstance.rotRate.y = Mathf.InverseLerp(boneInfoInstance.rotMin.y, boneInfoInstance.rotMax.y,
                        boneInfoInstance.baseRot.y);
                    boneInfoInstance.rotRate.z = Mathf.InverseLerp(boneInfoInstance.rotMin.z, boneInfoInstance.rotMax.z,
                        boneInfoInstance.baseRot.z);
                }

                var array = components;
                boneInfoInstance.dynamicBone = array.Where(n => !findAssist.DictObjName
                    .FirstOrDefault(x => x.Key == n.m_Root.name)
