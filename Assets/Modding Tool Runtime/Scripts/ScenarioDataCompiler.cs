...
namespace ModdingTool
{
    public static class ScenarioDataCompiler
    {
        ...
        {
            foreach (var textAsset in Selection.objects.OfType<TextAsset>()) CompileScenarioData(textAsset);
        }
        ...
        public static void CompileScenarioData(TextAsset textAsset)
        {
            ...
            var savePath = Path.Combine(PathUtils.GetProjectPath(),
                Path.GetFileNameWithoutExtension(textAsset.name) + ".asset").ToUnixPath();
            var random = new Random();
            var hashIndex = random.Next(int.MaxValue / 2);
