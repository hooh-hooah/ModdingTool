...
public static partial class MapInitializer
{
    ...
    {
        return Resources.Load<PostProcessProfile>(
                   volumeType == VolumeType.Default ? "hs2_default" : "hs2_default_color")
    ...
    private static void CreateDefaultPostProcessingVolume(GameObject postProcessingWrapper, VolumeInfo volumeInfo)
    {
        ...
        if (reflectionProbe != null) effectorVolume.rp = reflectionProbe;
    }

    private static void InitializePostProcessing(GameObject postProcessingWrapper)
    {
        foreach (var volumeInfo in VolumeNames) CreateDefaultPostProcessingVolume(postProcessingWrapper, volumeInfo);
    }
    ...
    private enum VolumeType { Default, Color }
}