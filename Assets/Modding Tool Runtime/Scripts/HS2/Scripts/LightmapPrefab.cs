...
public class LightmapPrefab : MonoBehaviour
{
    ...
    private void Setup()
    {
        var array = (from v in GetComponentsInChildren<Renderer>() where v.enabled && v.lightmapIndex >= 0 select v)
            .ToArray();
        ...
        for (var i = 0; i < array.Length; i++)
        {
            ...
            {
                lightmapIndex = renderer.lightmapIndex, scaleOffset = renderer.lightmapScaleOffset, renderer = renderer
            };
        }
    }
