...
class ManifestBuilder {
    ...
    public string Generate() {
        ...
        if (document.Root.Element("heels") != null) {
            IEnumerable<XElement> heels = document.Root.Element("heels").Elements("heel");
            int index = 0;
            ...
            if (heels != null && heels.Count() > 0) {
                ...
                foreach (var heel in heels) {
                    ...
                    #region FootInfo
                    XElement part; XElement mod;
                    foreach (var text in new string[] { "foot01", "foot02" }) {
                        part = heel.Element(text);
                        builder.AppendLine("\t\t\t<"+ text +"> ");
                            mod = part.Element("roll"); builder.AppendLine(string.Format("\t\t\t\t<roll vec=\"{0}\" min=\"{1}\" max=\"{2}\"/>", mod.Attribute("vec").Value, mod.Attribute("min").Value, mod.Attribute("max").Value));
                            mod = part.Element("move"); builder.AppendLine(string.Format("\t\t\t\t<move vec=\"{0}\"/>", mod.Attribute("vec").Value));
                            mod = part.Element("scale"); builder.AppendLine(string.Format("\t\t\t\t<scale vec=\"{0}\"/>", mod.Attribute("vec").Value));
                        builder.AppendLine("\t\t\t</" + text + ">");
                    }
                    #endregion
                    #region ToesInfo
                    ...
                    builder.AppendLine("\t\t\t<toes01>");
                        mod = part.Element("roll"); builder.AppendLine(string.Format("\t\t\t\t<roll vec=\"{0}\"/>", mod.Attribute("vec").Value));
                        mod = part.Element("move"); builder.AppendLine(string.Format("\t\t\t\t<move vec=\"{0}\"/>", mod.Attribute("vec").Value));
                        mod = part.Element("scale"); builder.AppendLine(string.Format("\t\t\t\t<scale vec=\"{0}\"/>", mod.Attribute("vec").Value));
                    builder.AppendLine("\t\t\t</toes01>"); 
                    #endregion
                    builder.AppendLine(string.Format("\t\t</heel>"));
                    index++;
                }
                builder.AppendLine("\t</AI_HeelsData>");
            }
