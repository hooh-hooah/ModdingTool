...
public static class ADVCommandParser
{
    ...
    {
        foreach (var file in Directory.GetFiles(path)) ParseADV(file);

        foreach (var directory in Directory.GetDirectories(path)) RecursiveDump(directory);
    }
    ...
    private static void ParseADV(string path)
    {
        ...
        {
            var targetPath = Path.Combine(output + Path.GetDirectoryName(dest), Path.GetFileNameWithoutExtension(dest),
                scenarioData.name + ".txt").ToUnixPath();
            Directory.CreateDirectory(Path.GetDirectoryName(targetPath));
            using (var stream = new StreamWriter(targetPath))
            {
                foreach (var param in scenarioData.list)
                    stream.WriteLine(
                        $"{Enum.GetName(typeof(Command), param._command)}\t{param._multi}\t{param._version}\t{string.Join("\t", param._args)}");
