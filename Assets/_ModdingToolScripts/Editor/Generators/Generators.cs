...
namespace Generator
{
    public partial class Generators : EditorWindow
    {
        ...
        {
            foreach (var controller in AssetDatabase.FindAssets("t:AnimatorController", new[] {PathUtils.GetProjectPath()})
        ...
        public static void GenerateAnimatorController()
        {
            ...
            {
                EditorUtility.DisplayDialog("Error", $"This folder does not have any folder has prefix '{AnimPrefix}'!", "I'll make one");
                return;
            }
            ...
            var animationClips = AssetDatabase.FindAssets("t:AnimationClip", animationFolders)
                ...
                .Select(assetPath =>
                {
                    if (assetPath.EndsWith(AnimExtension))
                        return new object[]
                        {
                            Path.GetDirectoryName(assetPath), new[] {AssetDatabase.LoadAssetAtPath<AnimationClip>(assetPath)}
                        };

                    var asset = AssetDatabase.LoadAllAssetRepresentationsAtPath(assetPath).OfType<AnimationClip>().ToArray();
                    return new object[]
                    {
            ...
            foreach (var pair in animationClips)
            {
                ...
                var controller = AnimatorController.CreateAnimatorControllerAtPath(Path.Combine(outputPath, controllerName));
            ...
            File.WriteAllText(Path.Combine(Directory.GetCurrentDirectory(), path, "generated_animations.txt"), builder.ToString());
        }
    }
}