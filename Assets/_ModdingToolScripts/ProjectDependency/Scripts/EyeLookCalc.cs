...
public class EyeLookCalc : MonoBehaviour
{
    ...
    private void OnDrawGizmos()
    {
        ...
        if (rootNode)
        {
            ...
            if (eyeTypeStates.Length > ptnDraw)
            {
                ...
                var vector = rootNode.TransformDirection(Quaternion.Euler(0f, eyeTypeState.hAngleLimit, 0f) * Vector3.forward * drawLineLength) + rootNode.position;
                Gizmos.DrawLine(rootNode.position, vector);
                var vector2 = rootNode.TransformDirection(Quaternion.Euler(0f, -eyeTypeState.hAngleLimit, 0f) * Vector3.forward * drawLineLength) + rootNode.position;
                Gizmos.DrawLine(rootNode.position, vector2);
                var vector3 = rootNode.TransformDirection(Quaternion.Euler(eyeTypeState.vAngleLimit, 0f, 0f) * Vector3.forward * drawLineLength) + rootNode.position;
                Gizmos.DrawLine(rootNode.position, vector3);
                var vector4 = rootNode.TransformDirection(Quaternion.Euler(-eyeTypeState.vAngleLimit, 0f, 0f) * Vector3.forward * drawLineLength) + rootNode.position;
                ...
                {
                    vector = eyeObjs[i].eyeTransform.TransformDirection(Quaternion.Euler(0f, eyeTypeState.minBendingAngle * (eyeObjs[i].eyeLR != EYE_LR.EYE_R ? -1 : 1), 0f) *
                                                                        Vector3.forward * drawLineLength) + eyeObjs[i].eyeTransform.position;
                    Gizmos.DrawLine(eyeObjs[i].eyeTransform.position, vector);
                    vector2 = eyeObjs[i].eyeTransform.TransformDirection(Quaternion.Euler(0f, eyeTypeState.maxBendingAngle * (eyeObjs[i].eyeLR != EYE_LR.EYE_R ? -1 : 1), 0f) *
                                                                         Vector3.forward * drawLineLength) + eyeObjs[i].eyeTransform.position;
                    Gizmos.DrawLine(eyeObjs[i].eyeTransform.position, vector2);
                    vector3 = eyeObjs[i].eyeTransform.TransformDirection(Quaternion.Euler(eyeTypeState.upBendingAngle, 0f, 0f) * Vector3.forward * drawLineLength) +
                              eyeObjs[i].eyeTransform.position;
                    Gizmos.DrawLine(eyeObjs[i].eyeTransform.position, vector3);
                    vector4 = eyeObjs[i].eyeTransform.TransformDirection(Quaternion.Euler(eyeTypeState.downBendingAngle, 0f, 0f) * Vector3.forward * drawLineLength) +
                              eyeObjs[i].eyeTransform.position;
            ...
            for (var j = 0; j < eyeObjs.Length; j++)
                Gizmos.DrawLine(eyeObjs[j].eyeTransform.position, eyeObjs[j].eyeTransform.position + eyeObjs[j].eyeTransform.forward * drawLineLength);
        }
    ...
    public void EyeUpdateCalc(Vector3 target, int ptnNo)
    {
        ...
        if (num < 0f || num3 < 0f || num2 > eyeTypeStates[ptnNo].hAngleLimit || num4 > eyeTypeStates[ptnNo].vAngleLimit) eye_LOOK_TYPE = EYE_LOOK_TYPE.FORWARD;
        if (eye_LOOK_TYPE == EYE_LOOK_TYPE.FORWARD) target = rootNode.position + rootNode.forward * eyeTypeStates[ptnNo].forntTagDis;
        ...
        foreach (var eyeObject in eyeObjs)
        {
            ...
            num6 = Mathf.Max(Mathf.Abs(f) * Mathf.Abs(eyeTypeState.bendingMultiplier), Mathf.Abs(num6) - eyeTypeState.maxAngleDifference) * Mathf.Sign(num6) *
                   Mathf.Sign(eyeTypeState.bendingMultiplier);
            num7 = Mathf.Max(Mathf.Abs(f2) * Mathf.Abs(eyeTypeState.bendingMultiplier), Mathf.Abs(num7) - eyeTypeState.maxAngleDifference) * Mathf.Sign(num7) *
                   Mathf.Sign(eyeTypeState.bendingMultiplier);
            ...
            if (eye_LOOK_TYPE == EYE_LOOK_TYPE.AWAY)
            {
                if (num5 == -1f)
                {
                    var num8 = Mathf.Lerp(-1f, 1f, Mathf.InverseLerp(-eyeTypeStates[nowPtnNo].maxBendingAngle, -eyeTypeStates[nowPtnNo].minBendingAngle, eyeObject.angleH));
                    var num9 = Mathf.Lerp(-1f, 1f, Mathf.InverseLerp(-eyeTypeStates[nowPtnNo].maxBendingAngle, -eyeTypeStates[nowPtnNo].minBendingAngle, num6));
                    var num10 = num8 - num9;
                    if (Mathf.Abs(num10) < sorasiRate)
                    {
                        ...
                        num6 = Mathf.Lerp(-eyeTypeStates[nowPtnNo].maxBendingAngle, -eyeTypeStates[nowPtnNo].minBendingAngle, num5);
                    }
                    else
                    {
                        num5 = Mathf.InverseLerp(-1f, 1f, num8);
            ...
            var lhs = rotation * Quaternion.LookRotation(forward, eyeObject.dirUp) *
                      Quaternion.Inverse(rotation * Quaternion.LookRotation(eyeObject.referenceLookDir, eyeObject.referenceUpDir));
        ...
        for (var i = 0; i < 2; i++)
            if (eyeObjs[i] != null)
            {
                if (eyeObjs[i].eyeLR == EYE_LR.EYE_R)
                    angleHRate[i] = Mathf.Lerp(-1f, 1f, Mathf.InverseLerp(-eyeTypeStates[nowPtnNo].maxBendingAngle, -eyeTypeStates[nowPtnNo].minBendingAngle, eyeObjs[i].angleH));
                else
    ...
    private float AngleVRateCalc()
    {
        ...
        {
            if (0f <= eyeObjs[0].angleV) return -Mathf.InverseLerp(0f, eyeTypeStates[nowPtnNo].upBendingAngle, eyeObjs[0].angleV);
            return Mathf.InverseLerp(0f, eyeTypeStates[nowPtnNo].downBendingAngle, eyeObjs[0].angleV);
        }

        if (0f <= eyeObjs[0].angleV) return -Mathf.InverseLerp(0f, eyeTypeStates[nowPtnNo].downBendingAngle, eyeObjs[0].angleV);
